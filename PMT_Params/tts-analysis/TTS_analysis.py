...
def single_tube(argv):
    ...
    for idx, afile in enumerate(allFiles[0:30]):
    ...
    plt.title("MCP PMT average all theta")
    plt.xlabel("phi")
    plt.ylabel("TTS")
    plt.savefig(argv[1])
...
def write_root(argv):
    ...
    hist_dyn = [TH1D("dyn%d"%i, "", 100, 0, 1000) for i in range(7) ]
    hist_nnvt = [TH1D("nnvt%d"%i, "", 100, 0, 1000)  for i in range(7) ]
    hist_dyn_phi = [ TH1D("dynPhi%d"%i, "", 100, 0, 1000) for i in range(24) ]
    hist_mcp_phi = [TH1D("nnvtPhi%d"%i, "", 100, 0, 1000) for i in range(24) ]
